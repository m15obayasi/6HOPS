...
初回認証と手動投稿の確認後、毎日9時に生成から公開指定投稿まで行うタスクを登録します。

```powershell
.\install-scheduled-task.ps1 -At '09:00' -Privacy public
```

パソコンが9時に停止していた場合は、次回起動後に実行されます。経路を実際のWikipediaリンクで検証できない場合や動画生成に失敗した場合は、誤った動画を投稿せず停止します。ログは `daily-logs` に保存されます。

$(document).ready(function() {
    ...
    $('.homeLink').click(function() {
        if (confirm('Zurück zur Startseite?')) {
...
function fetchRandomWikipediaTitles() {
    $.ajax({
        ...
        error: function(error) {
            ...
            alert('Fehler beim Abrufen zufälliger Artikeltitel.');
        }
    });
}
...
function showActionButtons() {
    ...
    const retryButton = $('<button class="button retry">Noch einmal</button>');
    const confirmButton = $('<button class="button confirm">Bestätigen</button>');
...
function fetchGoalSummary(title) {
    $.ajax({
        ...
        error: function(error) {
            ...
            alert('Fehler beim Abrufen der Zusammenfassung des Zielartikels.');
        }
    });
}
...
function setupLinkClickHandlers() {
    ...
    $('.wikiBlock').on('click', 'a', function(event) {
        ...
        if (linkTitle) {
            // Check if the link is to a non-existent or non-German article
...
function displayResult(result) {
    const resultText = result === 'Erfolg' ? 'Erfolg' : 'Fehlschlag';
    const resultMessage = result === 'Erfolg' ? 'Herzlichen Glückwunsch! Sie haben den Zielartikel erreicht!' : 'Leider haben Sie den Zielartikel nicht innerhalb von 6 Klicks erreicht...';
    const shareMessage = result === 'Erfolg' 
    ...
    let continueButton = '';
    if (result === 'Fehlschlag') {
        continueButton = `<div class="continueButton">Trotzdem fortfahren</div>`;
    }
